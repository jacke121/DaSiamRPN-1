...
for dir_name in dirs:
    ...
    for epoch in range(config["epochs"]):
        ...
        while True:
            ...
            # nn.MSELoss(size_average=False).cuda()
            # mse_loss0 = nn.MSELoss()(torch.Tensor(a[0]),torch.Tensor(b[0]))
            mse_loss0 = nn.MSELoss(size_average=False)(torch.Tensor(a[0]),torch.Tensor(b[0]))
            # mse_loss1 = nn.MSELoss()(torch.Tensor(a[1]),torch.Tensor(b[1]))
            # mse_loss2 = nn.MSELoss()(torch.Tensor(a[2]),torch.Tensor(b[2]))
            # mse_loss3 = nn.MSELoss()(torch.Tensor(a[3]),torch.Tensor(b[3]))

            recall=recall+iou
            optimizer.zero_grad()
            # loss =cross(target,iou).cuda()
            loss =(mse_loss0/3000+(myloss(target,iou)*10)).cuda()
            # loss =(mse_loss0/36/48+ (myloss(target,iou)*5)).cuda()
            # loss =(mse_loss0+mse_loss1+mse_loss2+mse_loss3+ (myloss(target,iou)*10)).cuda()
            print(frame, time.time() - start,loss.item(),iou,epoch)
            loss.backward()
            optimizer.step()
            _loss = loss.item()
